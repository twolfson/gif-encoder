...
function GIFEncoder(width, height, options) {
  ...
  if (options.palette !== undefined) {
    this.userPalette = options.palette;
    this.palSize = Math.log2(options.palette.length / 3) - 1;
  }
...
GIFEncoder.prototype.analyzeImage = function (imageData, options) {
  ...
  if (options && options.indexedPixels === true) {
    assert(this.userPalette, '`options.indexedPixels` can only be used with a `palette` at the moment');
...
GIFEncoder.prototype.analyzePixels = function(options) {
  ...
  // color remapping.
  var nPix, index, imgq;
  if (this.userPalette !== undefined) {
    assert(options && options.indexedPixels === true, '`palette` can only be used with `options.indexedPixels`. ' +
      'Please add `{indexedPixels: true}` to your `addFrame` invocation');
