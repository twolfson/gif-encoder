...
ByteCapacitor.prototype._read = function (size) {
  var data = this.data;
  var len = data.length;

  // If there is no data to push, push null (marks end of stream)
  if (len === 0) {
    this.push(null);
  }

  // By default, output all of data
  var toOutput = len;

  // If size is provided
  if (size) {
    // Output as much as we can but nothing beyond available data set
    toOutput = Math.min(size, len);
  }

  // Remove the elements
  var bytes = data.splice(0, toOutput);
  this.push(new Buffer(bytes));
};
